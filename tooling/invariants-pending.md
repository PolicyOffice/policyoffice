...
- INV-TIME-004 — MVP; the request layer landed with POL-033, so "no request layer exists to retry" stopped being true. `apps/web/src/authoring.ts` serves create, draft and submit, and no route under `apps/web/src` carries an idempotency key: a retried Save draft mints a fresh `revisionId` and `requestId` and writes a second `content_revision` instead of returning the first result. What it will build on is in place — `transition_document_version_effective` is idempotent by construction (`0013`) and audit emission carries a `dedupeKey`. Live once a route accepts a client-supplied key and stores it
