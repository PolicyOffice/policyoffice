...
| 10 | `Published` | `Withdrawn` | Withdrawn before taking effect | `document.withdraw` | Reason required. `effective_until` is set to `effective_from`, producing an empty range: the version never becomes effective, including under a race with the scheduler | `version.withdrawn` |
...
| 12 | `Effective` | `Withdrawn` | Deliberate withdrawal from effect | `document.withdraw` | Reason required. `effective_until` is shortened to the transaction instant, preserving elapsed governance. No predecessor is resurrected (INV-EFF-004). If no other version is effective for the scope, a high-severity `governance.policy_gap` event is emitted (INV-EFF-005) | `version.withdrawn`, possibly `governance.policy_gap` |
