...
1. **POL-039** — approval runs start at submission. The next ticket to write: POL-038 merged as
   #149 on 2026-09-17, so the template tables, the Pilot parsers and the satisfaction relation it
   rests on are in `main`.
...
| **POL-039** | Runs start at submission: participants resolved and frozen (INV-APR-012), the mandate checked again at run start (INV-APR-020), `approval_run.started`, `approval_stage.started`, `approval_task.assigned` | **next to write** |
